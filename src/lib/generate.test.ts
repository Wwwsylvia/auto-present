...
import {
  ...
  foundryCompletionAdapter,
  normalizeSlideDurations,
  type Completion,
  type CompletionRequest,
} from "@/lib/generate";

test("preserves the typed Foundry chat boundary for response fixtures", async () => {
  let model = "";
  const completion = foundryCompletionAdapter("fixture-deployment", async (request) => {
    model = request.model;
  ...
  const raw = await completion({
    pass: "draft",
    system: "Return JSON",
    user: JSON.stringify({ evidence: "untrusted fixture context" }),
    attempt: 1,
  });

  assert.equal(model, "fixture-deployment");
  assert.deepEqual(JSON.parse(raw), responses.validGeneration);
});
