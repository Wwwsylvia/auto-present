...
export async function renderPresentation(
  ...
  for (const [index, slide] of revision.slides.entries()) {
    ...
    if (hasSpeech) {
      ...
      const timingScale = duration / audioDuration;
      captions.push(
        ...sentenceCaptionCues(slide.narration, boundaries, audioDuration).map((cue) => ({
          ...cue,
          startSeconds: cue.startSeconds * timingScale + elapsed,
          endSeconds: Math.min(duration, cue.endSeconds * timingScale) + elapsed,
        })),
      );
      await run(
        "ffmpeg",
