...
test("legacy completed renders remain downloadable without a status sidecar", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "idea2impact-legacy-download-"));
  const previous = process.env.IDEA2IMPACT_DATA_DIR;
  process.env.IDEA2IMPACT_DATA_DIR = directory;
  const id = "46f7f12c-945e-47b3-9362-cbdcd0f1833b";
  try {
    const project = await createProject({
      idea: "A sufficiently detailed legacy render compatibility project.",
      audience: "Judges",
      tone: "confident",
      durationMinutes: 2,
      githubUrl: "",
    ...
    const output = path.join(directory, "renders", id, "presentation.mp4");
    await mkdir(path.dirname(output), { recursive: true });
    await writeFile(output, "legacy-video");
    ...
    assert.equal(Buffer.from(await response.arrayBuffer()).toString(), "legacy-video");
  } finally {
    if (previous === undefined) delete process.env.IDEA2IMPACT_DATA_DIR;
    else process.env.IDEA2IMPACT_DATA_DIR = previous;
    await rm(directory, { recursive: true, force: true });
